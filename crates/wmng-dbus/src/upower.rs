...
impl UPower {
    ...
    pub async fn receive_display_device_changed(&self) -> Result<PropertiesChangedStream> {
        ...
        Ok(self.properties_proxy(path.as_str()).await?.receive_properties_changed().await?)
    }

    async fn proxy(&self) -> Result<UPowerManagerProxy<'_>> {
    ...
    async fn display_device_proxy(&self, object_path: &str) -> Result<UPowerDeviceProxy<'_>> {
    ...
    async fn properties_proxy(&self, object_path: &str) -> Result<PropertiesProxy<'_>> {
