...
impl UDisks2 {
    ...
    pub async fn receive_interfaces_added(&self) -> Result<InterfacesAddedStream> {
        Ok(self.object_manager().await?.receive_interfaces_added().await?)
    }
    ...
    pub async fn receive_interfaces_removed(&self) -> Result<InterfacesRemovedStream> {
        Ok(self.object_manager().await?.receive_interfaces_removed().await?)
    }
    ...
    async fn filesystem_proxy(&self, object_path: &str) -> Result<FilesystemProxy<'_>> {
...
fn block_device_from_interfaces(
    ...
    let drive_path = normalize_string(optional::<OwnedObjectPath>(block, BLOCK_IFACE, "Drive")?.map(
        |path| path.to_string(),
    ));
...
mod tests {
    ...
    use super::{
        parse_managed_objects, BlockDeviceSnapshot, BLOCK_IFACE, DRIVE_IFACE, FILESYSTEM_IFACE,
    };
    ...
    fn parse_block_device_snapshot() {
        ...
        block.insert("Device".into(), OwnedValue::from(b"/dev/sdb1\0".to_vec()));
        block.insert("PreferredDevice".into(), OwnedValue::from(b"/dev/disk/by-uuid/demo\0".to_vec()));
        block.insert(
            "Drive".into(),
            OwnedValue::from(ObjectPath::try_from("/org/freedesktop/UDisks2/drives/usb0").unwrap()),
        );
        block.insert("IdUsage".into(), OwnedValue::from(String::from("filesystem")));
        block.insert("IdType".into(), OwnedValue::from(String::from("vfat")));
        ...
        filesystem.insert(
            "MountPoints".into(),
            OwnedValue::from(vec![b"/run/media/demo\0".to_vec()]),
        );
        ...
        drive.insert("ConnectionBus".into(), OwnedValue::from(String::from("usb")));
